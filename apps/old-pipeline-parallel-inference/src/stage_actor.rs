...
pub struct StageActor {
    ...
    /// Bounded ring of recent stderr lines from the worker subprocess. Drained
    /// to this process's own stderr if the worker exits abnormally, so a crash
    /// is self-diagnosing without any telemetry backend.
    ...
    stderr_buf: String,
    /// Most recent worker `uncaught_exception` traceback (if any). Stashed here
    /// so the abnormal-exit stderr mirror can print it alongside the stderr tail
    /// even when the per-line event has scrolled past.
...
impl StageActor {
    ...
    fn handle_worker_line(&mut self, ctx: &Ctx, line: &str) {
        ...
        // Worker-side lifecycle event: a `{"event": "<kind>", ...}` line. The
        // only one we act on is `uncaught_exception` — stash its traceback so
        // the abnormal-exit stderr mirror can print it alongside the stderr tail
        // even when the per-line event has scrolled past. Other event lines are
        // informational and consumed silently.
        ...
        match msg {
            ...
            StageMsg::Process(notif) => match notif {
                ...
                ProcessNotification::Exited { status, process } => {
                    ...
                    // stderr. The worker's stderr is otherwise consumed here for
                    // the protocol parser; pp-worker's stderr is captured by the
                    // container log, so echoing the tail makes a crashed worker
                    // self-diagnosing with no telemetry backend involved.
                    if !normal_exit {
                        eprintln!(
