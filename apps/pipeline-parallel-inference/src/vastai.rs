...
///
/// Fleet telemetry no longer rides a dedicated channel: each `pp-worker` ships
/// it over the swactor cluster transport to the orchestrator's `datastream-sink`
/// actor, so there is nothing fleet-related to inject here. What remains is the
/// custom iroh relay (`SWACTOR_IROH_RELAY_URL`) every stage dials to join SWIM
/// across the internet, and the orchestrator's deploy SSH public key
/// (`PP_DEPLOY_PUBKEY`) the container authorizes so the orchestrator can SSH in
/// to drive the boot. `create_instance` injects these alongside the per-stage
/// `STAGE` / `NUM_STAGES`.
#[derive(Debug, Clone, Default)]
pub struct StageEnv {
    /// Custom iroh relay URL stages dial to reach the orchestrator across the
    /// internet. Load-bearing for vast.ai WAN convergence.
    pub iroh_relay_url: Option<String>,
    /// Orchestrator's deploy SSH public key, injected as `SSH_PUBLIC_KEY` so the
    /// container's sshd authorizes the orchestrator. Required for the SSH-driven
    /// boot (the orchestrator launches `pp-worker` over SSH and streams its
    /// output onto its own datastream).
    pub ssh_public_key: Option<String>,
}

impl StageEnv {
    /// Read the relay URL + deploy SSH public key from the current process env.
    /// Returns an instance with all fields `None` when nothing is set — callers
    /// can still pass it and `create_instance` will skip the injection.
...
/// the new node knows where to join the SWIM cluster. When `stage_env` is
/// `Some(..)`, its fleet sink + relay URL are injected so the rented container
/// ships fleet telemetry to the orchestrator and joins over the right relay —
/// see [`StageEnv`].
pub async fn create_instance(
    ...
    if let Some(stage_env) = stage_env {
        ...
        // Authorize the orchestrator's deploy key so it can SSH in to drive the
        // boot. The entrypoint appends SSH_PUBLIC_KEY to authorized_keys
        // alongside vast's own account key.
        if let Some(pubkey) = stage_env.ssh_public_key.as_deref() {
            env["SSH_PUBLIC_KEY"] = serde_json::Value::String(pubkey.to_string());
        }
    }
