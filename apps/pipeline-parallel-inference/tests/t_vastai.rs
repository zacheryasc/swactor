...
/// When a `StageEnv` is supplied, every create_instance PUT must carry the
/// custom relay URL and the orchestrator's deploy SSH public key (`SSH_PUBLIC_KEY`)
/// into the rented container's env. Fleet telemetry no longer needs an env var
/// (it rides the cluster transport); the deploy key authorizes the orchestrator
/// to SSH in and stream the boot log.
#[tokio::test]
async fn create_instances_forward_stage_env_to_each_stage_container() {
    let server = MockServer::start().await;
    ...
    let stage_env = vastai::StageEnv {
        ...
        ssh_public_key: Some("ssh-ed25519 AAAAdeploykey orchestrator".into()),
    };
    vastai::create_pipeline_instances(
        &client,
        &server.uri(),
    ...
    for i in 0..3u32 {
        ...
        assert_eq!(env["NUM_STAGES"], "3");
    }
}

/// Conversely: without a `StageEnv` (None), no fleet/relay keys appear in the
/// create_instance payload — confirming the forwarding is fully opt-in and
/// doesn't leak the orchestrator's local setup into runs that didn't ask for it.
#[tokio::test]
async fn create_instances_omit_stage_env_when_not_supplied() {
    let server = MockServer::start().await;
    ...
    for req in server.received_requests().await.unwrap() {
        ...
        let body: serde_json::Value = serde_json::from_slice(&req.body).unwrap();
        for key in ["SSH_PUBLIC_KEY", "SWACTOR_IROH_RELAY_URL"] {
            assert!(
                body["env"].get(key).is_none(),
                "env must not carry {key} when no StageEnv supplied; got {:?}",
                body["env"],
            );
        }
    }
