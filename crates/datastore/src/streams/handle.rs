...
use crate::streams::buffer::{BufferPool, FrameBuf};
use crate::streams::channel::{RecvCommand, RecvEvent, SendCommand, SendEvent};
use crate::streams::types::{StreamConfig, StreamError, StreamId};
...
impl SendHalf {
    ...
    pub fn flush(&mut self) -> Result<(), StreamError> {
        if let Some(buf) = self.active_buf.take() {
            if buf.remaining() > 0 || !buf.written().is_empty() {
                self.cmd_tx
