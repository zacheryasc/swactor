...
//!
//! The driver runs on an ambient tokio runtime via a [`Handle`] (it owns no
//! runtime of its own). It ferries frames between iroh and the swactor runtime:
//! [`IrohDriver::pump_inbound_to_actors`] decodes received frames into actor
//! mailboxes and [`IrohDriver::drain_outbox`] writes actor-produced frames out.
//! Those are pure-sync (no `block_on`), so the node's async driver loop calls
//! them on a tokio worker; the constructor and [`IrohDriver::shutdown`] still
//! `block_on` and must run on a non-async thread (the async loop uses
//! [`IrohDriver::close`]).
...
use crate::messages::*;
use crate::node::DistributedNodeConfig;
...
/// to iroh's async QUIC transport. Runs on an ambient tokio runtime via a
/// [`Handle`]; it owns no runtime of its own. [`Self::pump_inbound_to_actors`]
/// and [`Self::drain_outbox`] are pure-synchronous (they drain in-memory queues
...
pub struct IrohDriver {
    ...
    /// A tokio runtime owned by this driver, present only when [`Self::new`] was
    /// called outside any tokio context and had to create one. When the driver
    /// runs on an ambient/shared runtime (the node's single-engine path via
    /// [`Self::with_handle`], or `new()` inside `#[tokio::main]`) this is `None`.
...
impl IrohDriver {
    /// Create a new iroh driver, adapting to the caller's tokio context.
    ///
    /// - **Inside a tokio runtime** (the node's single-engine path, or
    ///   `#[tokio::main]`): the driver shares that ambient runtime — no second
    ///   engine is created.
    /// - **Outside any tokio runtime** (a plain synchronous `main`, e.g. a
    ///   standalone binary): the driver creates and owns a multi-threaded
    ///   runtime, preserving the zero-setup behaviour callers relied on before
    ///   the driver stopped unconditionally owning one.
    ///
    /// Either way the synchronous facade (`recv`/`tick`/`snapshot`/`shutdown`)
    /// must be driven from a non-async thread (see [`Self::with_handle`]). To
    /// force sharing of a specific runtime, use [`Self::with_handle`].
    ...
    /// the signed [`DirectoryEntry`](crate::types::DirectoryEntry) verifies
    ...
    pub fn drain_outbox(&mut self, outbox: &Outbox) {
        let frames: Vec<crate::transport_bridge::OutFrame> =
