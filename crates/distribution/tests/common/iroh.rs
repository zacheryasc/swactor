...
//! `DirectoryActor`. This mirrors production (`crates/node/src/main.rs`): the
//! driver decodes inbound frames into actor mailboxes, the actors enqueue
//! outbound frames on a shared [`Outbox`], and the driver writes them to iroh.
//!
...
impl IrohNode {
    /// Build a node from a driver config, mirroring the production wiring in
    /// `crates/node/src/main.rs` (build outbox/relay_mirror/route_view;
    /// `IrohPeerDirectory::new(router, outbox)`; spawn the 4 actors;
