//! `iroh-driver-relay` — standalone iroh-relay server.
//!
...
fn print_help() {
    eprintln!(
        "Usage:\n  \
         iroh-driver-relay [--bind ADDR] [--public-host HOST]\n\n\
         Options:\n  \
...
async fn main() -> ExitCode {
    ...
    let server =
        ...
        .await
        {
            Ok(s) => s,
            Err(e) => {
                eprintln!("iroh-driver-relay: failed to spawn relay server: {e}");
    ...
    let addr = match server.http_addr() {
        Some(a) => a,
        None => {
            eprintln!("iroh-driver-relay: relay has no HTTP address");
            return ExitCode::from(1);
        }
    };
    ...
    eprintln!(
        "iroh-driver-relay: listening on {bind} (advertised URL: {url}); \
    ...
    loop {
        tokio::select! {
            res = tokio::signal::ctrl_c() => {
                if let Err(e) = res {
                    eprintln!("iroh-driver-relay: signal listen failed: {e}");
                    return ExitCode::from(1);
                }
                break;
            }
    ...
    eprintln!("iroh-driver-relay: shutdown signal received");
    ExitCode::SUCCESS
}
