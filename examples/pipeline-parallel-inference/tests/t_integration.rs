...
/// Build a `num_stages + 1`-node cluster: driver `[0]` is the orchestrator,
/// drivers `[1..=num_stages]` are pipeline stages `0..num_stages-1`. Every
...
/// runs with exclusive access to the localhost network stack and
/// completes inside the timeout below even under default-parallel
/// `cargo test`.
fn make_cluster(num_stages: u32) -> Vec<IrohDriver> {
    ...
    // 60s is generous with the semaphore bounding fan-out — exclusive
    // convergence finishes in under 5s on this box; the cap is sized
    // for a slow CI runner where ambient pump activity in the other
    // in-flight tests can slow the handshake but no longer dominates
    // the way it did at default `cargo test` parallelism.
...
fn build_pipeline(num_stages: u32, max_tokens: u32, eos: Option<u32>, observe_tokens: bool) -> Pipeline {
    ...
    let drivers = make_cluster(num_stages);
    ...
    let status_inboxes: Vec<Inbox<StageActorStatus>> = (0..num_stages)
        .map(|s| runtimes[s as usize + 1].new_inbox::<StageActorStatus>().unwrap())
        .collect();
    ...
    let (token_observer_inbox, observer_addr): (Option<Inbox<NextToken>>, Option<ActorAddress>) = if observe_tokens {
        let inbox = runtimes[last_stage as usize + 1].new_inbox::<NextToken>().unwrap();
        let addr = *inbox.addr();
    ...
    // Spawn each stage actor on its own runtime. The First's `next_stage`
    ...
    let request_bridge_addr = runtimes[1]
        .spawn(RequestBridge { target: stage_actor_addrs[0] })
        .unwrap();
    let nt_bridge_addr = runtimes[1]
    ...
    // - First: next_stage = stage 1's activation bridge.
    // - Middle s: next_stage = stage s+1's activation bridge.
    // - Last: prev_stage = stage 0's nt_bridge_addr. reply_to is already
    //   set via constructor.
    for s in 0..num_stages {
        let rt = &runtimes[s as usize + 1];
    ...
    // Transport routes. Each runtime needs routes for every outbound
    // ActorAddress it sends to that lives on a different node.
    //
    ...
    runtimes[0].set_codec_registry(codecs.clone());
    runtimes[0].set_transport_router(Arc::new(router_orch));
    ...
    for s in 0..num_stages - 1 {
        ...
        runtimes[rt_idx].set_codec_registry(codecs.clone());
        runtimes[rt_idx].set_transport_router(Arc::new(router));
    }
    ...
    router_last.add_route(inbox_addr, last_to_orch);
    runtimes[last_idx].set_codec_registry(codecs.clone());
    runtimes[last_idx].set_transport_router(Arc::new(router_last));
...
impl Pipeline {
    ...
    fn pump(&mut self) {
        ...
        for (i, pump) in self.pumps.iter().enumerate() {
            pump.pump(&self.drivers[i], &self.codecs, &self.runtimes[i]);
        }
    ...
    fn pump_orch_only(&mut self) {
        self.drivers[0].recv();
        self.drivers[0].tick();
        self.pumps[0].pump(&self.drivers[0], &self.codecs, &self.runtimes[0]);
        self.runtimes[0].tick();
    }
    ...
    fn await_response_or_stage_failure(
        ...
        while start.elapsed() < timeout {
            ...
            let snap = self.drivers[0].snapshot();
    ...
    fn shutdown(mut self) {
        for (s, addr) in self.stage_actor_addrs.iter().enumerate() {
            let _ = self.runtimes[s + 1].stop_actor(*addr);
        }
        let deadline = Instant::now() + Duration::from_secs(3);
        while Instant::now() < deadline {
            for s in 0..self.num_stages as usize {
                self.runtimes[s + 1].tick();
            }
...
fn stage_failure_mid_decode_case(victim_stage: u32) {
    ...
    kill_pid(victim_pid);
    pipeline.drivers[victim_stage as usize + 1].shutdown();

    // SwimConfig: probe_timeout=3, suspicion_timeout=5 → dead within
    ...
    for s in 0..num_stages {
        if s != victim_stage {
            let _ = pipeline.runtimes[s as usize + 1].stop_actor(pipeline.stage_actor_addrs[s as usize]);
        }
    }
    ...
    while Instant::now() < deadline {
        for s in 0..num_stages {
            if s != victim_stage {
                pipeline.runtimes[s as usize + 1].tick();
            }
        }
    ...
    for (i, d) in pipeline.drivers.iter_mut().enumerate() {
        // Driver at `victim_stage + 1` is already shut down.
...
fn build_real_pipeline(num_stages: u32, max_tokens: u32) -> Pipeline {
    ...
    let token_observer_inbox = runtimes[last_stage as usize + 1]
        .new_inbox::<NextToken>()
        .unwrap();
    ...
    let rts_refs: Vec<&Runtime> = runtimes.iter().skip(1).collect();
    let mut stage_pids: Vec<u32> = Vec::with_capacity(num_stages as usize);
    for s in 0..num_stages {
...
impl Pipeline {
    ...
    fn reset_for_next_prompt(&mut self) {
        for (s, addr) in self.stage_actor_addrs.iter().enumerate() {
            self.runtimes[s + 1].send_to(*addr, StageMsg::Reset).unwrap();
        }
        for _ in 0..5 {
            self.pump();
            std::thread::sleep(Duration::from_millis(10));
