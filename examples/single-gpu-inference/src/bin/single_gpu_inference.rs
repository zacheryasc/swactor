...
fn run_localhost(seed_hex: &str, seed_direct: &[SocketAddr]) {
    ...
    // Create the local actorized cluster node (driver bridge + protocol actors).
    ...
    while start.elapsed() < Duration::from_secs(30) {
        ...
        if let Some((addr, _node_id)) = cluster.resolve_name("inference") {
    ...
    // Response inbox on the cluster runtime.
    let codecs = Arc::new(inference_codec_registry());
    let response_inbox = cluster.rt.new_inbox::<InferenceResponse>().unwrap();
    let inbox_addr = *response_inbox.addr();

    // Build transport to the remote node and add the route on the cluster's
    // shared transport router (the inference send path).
    let transport_to_remote = Arc::new(IrohActorTransport::new(
        cluster.driver.endpoint().clone(),
        seed_addr,
        cluster.driver.tokio_handle(),
    ));
    cluster.transport_router.add_route(bridge_addr, transport_to_remote);
...
fn run_vastai(api_key_raw: String, gpu_name: &str, image: &str) {
    ...
    // Create local cluster node first so we know our node id.
    ...
    let relay_url = {
        ...
        loop {
            cluster.pump_once();
            if let Some(url) = cluster.driver.home_relay_url() {
    ...
    while start.elapsed() < Duration::from_secs(60) {
        cluster.pump_once();
        if let Some((addr, node_id)) = cluster.resolve_name("inference") {
            bridge_addr = Some(addr);
